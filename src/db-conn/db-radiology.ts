...
export const db37AppointPool = new sql.ConnectionPool({
  server: process.env.DB_RAD_37_IP || "localhost",
  user: process.env.DB_RAD_37_USERNAME,
  port: Number(process.env.DB_RAD_37_PORT) || undefined,
  password: process.env.DB_RAD_37_PASSWORD,
  database: process.env.DB_RAD_37_DATABASE_APPOINTMENT,
  options: { enableArithAbort: true, encrypt: false },
});

export const db37AppointCon = db37AppointPool.connect().then((e) => {
  if (process.env.NODE_ENV !== "test" && process.env.CI !== "true")
    console.info("SQL server .37 connected");
  return e;
});
...
if (
  ...
  process.env.DB_RAD_37_DATABASE_APPOINTMENT === undefined ||
  process.env.DB_RAD_37_DATABASE_APPOINTMENT_SCHEMA === undefined
)
